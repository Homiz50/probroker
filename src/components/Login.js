  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_IP}/user/v2/login/dljcnji/cekbjid`,
        {
          number,
          password,
    ...
    <div className="flex h-screen justify-center items-center bg-[#FAF7FF] mx-4 lg:mx-28">
      ...
      <div className="flex-1 max-w-lg p-12 bg-white rounded-lg shadow-lg ">
        ...
        <form onSubmit={handleSubmit}>
          ...
          By clicking on login button you're accepting our <Link to="/terms-and-condition" className="text-blue-500"> terms and conditions</Link> and   <Link to="/privacy-policy" className=" text-blue-500" >privacy policy</Link>
          </p>
          <button
            type="submit"
          ...
          <div className="text-center mt-4">
            <p>
              Not a member?{" "}
              <Link to="/signup" className="text-[#503691] font-semibold">
                Signup
              </Link>
            </p>
          </div>
